...
#else
#define VSET1(e, m) asm volatile("th.vsetvli x0, %[sz], " #e ", " #m ::[sz] "r"(1));
#endif

inline float vmaxabs(size_t sz, const float* t, float init) {
    VSET1(e32, m1);
    asm volatile("th.vfmv.s.f v1, %[init]\n" : : [init] "f"(init));
    size_t lmul = mk_lmul(E32, sz);
    size_t vt8 = mk_vtype(E8, lmul), vt32 = mk_vtype(E32, lmul);
    for (; sz > 0;) {
        int vl;
        asm volatile(
                "th.vsetvl        x0, %[sz8], %[vt8]\n"
                "th.vlbu.v        v8, (%[t])\n"
                "th.vsetvl        %[vl], %[sz32], %[vt32]\n"
                "th.vfsgnjx.vv    v8, v8, v8\n"
                "th.vfredmax.vs   v1, v8, v1\n"
                : [vl] "=r"(vl)
    ...
    VSET1(e32, m1);
    asm volatile("th.vfmv.f.s  %[init],  v1\n" : [init] "=f"(init));
    return init;
}
...
inline float vmulsum(const float* x, const float* y, int sz, float init) {
    ...
    for (; sz > 0;) {
        ...
        asm volatile(
                "slli          t0, %[sz], 2\n"
                "th.vsetvli       t0, t0, e8, m8\n"
                "th.vlbu.v        v8,  (%[x])\n"
                "th.vlbu.v        v16, (%[y])\n"
                "srli          t0, t0, 2\n"
                "th.vsetvli       %[vl], t0, e32, m8\n"
                "th.vfmul.vv      v8, v8, v16\n"
                "th.vfredsum.vs   v1, v8, v1\n"
                : [vl] "=r"(vl)
                : [sz] "r"(sz), [x] "r"(x), [y] "r"(y)
                : "t0", "memory");
        x += vl;
...
inline void vscal(int sz, const float* x, float* z, float scale) {
    for (; sz > 0;) {
        ...
        asm volatile(
                ...
                "th.vlbu.v        v8, (%[x])\n"
                "srli          t1, t0, 2\n"
                "th.vsetvli       %[vl], t1, e32, m8\n"
                "th.vfmul.vf      v8, v8, %[scale]\n"
                "th.vsetvli       x0, t0, e8, m8\n"
                "th.vsb.v         v8, (%[z])\n"
                : [vl] "=r"(vl)
